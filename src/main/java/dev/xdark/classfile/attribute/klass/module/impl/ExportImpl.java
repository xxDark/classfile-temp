...
public final class ExportImpl implements Export {

	private final int packageIndex;
	private final int flags;
	private final int[] moduleIndices;

	public ExportImpl(int packageIndex, int flags, int[] moduleIndices) {
	...
	public static Codec<Export> codec() {
		return Codec.wire(
				Input.wire(reader -> {
					return new ExportImpl(reader.readConstantPoolIndex(), reader.readAccessFlags(), AttributeHelper.readUnsignedShorts(reader));
